...
func _ready():
	EventBus.subscribe_many(["battle_started"], [view, turn_timer, simulator])
	...
	if not Situation.level_data.player_team:
		Situation.level_data.player_team = RunManager.heroes
	...
	for x in range(Situation.player_team_data.size()):
		Utils.HeroToFighter(Situation.level_data.player_team[x], Situation.fighters[x])
	for x in range(4, 8):
		match x:
			4:
